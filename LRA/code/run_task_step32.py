...
print(lra_config_step32.config[task]["extra_attn_config"].keys(), flush=True)

model_config = lra_config_step32.config[task]["model"]
model_config.update(
    lra_config_step32.config[task]["extra_attn_config"][attn_type])
...
training_config = lra_config_step32.config[task]["training"]
gpu_memory_config = lra_config_step32.config[task]["gpu_memory"]
...
log_f_path = os.path.join(
    checkpoint_dir, f"{date}_{task}_{attn_type}_output_random32.log")
